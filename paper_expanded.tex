...
A rank-$r$ orthogonal projector $P^\star$ has \emph{finite global
privilege} for the family $\mathcal{T}$ under score $S$ if
\begin{equation}
...
then $V^\star$ is a Ky--Fan maximiser for every $T_t$, attaining
$\sum_{i=1}^r\lambda_i(T_t)$.
A Ky--Fan-maximising $r$-subspace must contain at least one unit vector
from $\Emax(T_t)$: if $V^\star\cap\Emax(T_t)=\{0\}$ then every unit vector
in $V^\star$ has Rayleigh quotient strictly less than $\lambda_1(T_t)$,
and replacing any such vector by a unit vector in $\Emax(T_t)$ strictly
increases the trace, contradicting optimality.
Hence $V^\star\cap\Emax(T_t)\ne\{0\}$ for every $t$, and $V^\star$ is an
$r$-cover.
...
and vice versa: the existence law for a common optimal projection depends
on the task functional.
\end{remark}
