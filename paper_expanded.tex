...
When coverability fails over a finite-support regime of local PSD
response operators $\{A_i\}$, the eigenvalue-weighted surrogate
...
causal landscape; we prove a sandwich
...
that is tight in three structural regimes (rank-$1$ $A_i$,
$r$-coverable case, common-shape family).
...
The LGDS Bayes-risk instance is recovered as a corollary.
...
causal/operator-norm targets $\{K_t\}$ if and only if the family is
\emph{$r$-coverable}: some rank-$r$ subspace intersects $\Emax(K_t)$
nontrivially for every $t$.
``Causal'' here refers to local linearised intervention response
(Section~\ref{sec:caus}), not to statistical causal identification.
...
\item a \emph{causal/operator-norm target operator} $K_t \succeq 0$.
\end{itemize}
``Causal'' here is shorthand for the local linearised
intervention-response setting derived in Section~\ref{sec:caus};
it is not intended to denote statistical causal identification, do-calculus,
or counterfactual inference.
...
The causal score $\lambda_{\max}(PK_tP)$ is not a Ky Fan trace.
...
predictive ($M_t$) vs.\ causal ($K_t$) interpretations of
...
We emphasise that the bounds developed here control the \emph{value
regret} $F(P_F^\star)-F(P_B^\star)$, not the subspace distance
...
F(P) &:= \sum_{i=1}^N w_i\,\lambda_{\max}(P A_i P)
        =\sum_{i=1}^N w_i\,S^{\mathrm{caus}}_i(P), \label{eq:F}\\
\tilde F(P) &:= \tr(P B_\lambda),\qquad
B_\lambda := \sum_{i=1}^N w_i\,\lambda_1(A_i)\,u_i u_i^\top.
\label{eq:Btilde}
\end{align}
Thus $F$ is the $\mu$-weighted average of the per-target causal score
$S^{\mathrm{caus}}_i(P)$ from~\eqref{eq:scores} with $K_i:=A_i$.
...
giving $\Gamma_{\mathrm{pred}}^{\mathrm{fork}}=2$, attained by
...
open and structurally harder than its predictive analogue
(Corollary~\ref{cor:approxcoh}): predictive regret has a smooth,
principal-angle-controlled bound because the Ky Fan trace is analytic in
$P$, whereas causal regret under $\lambda_{\max}(PKP)$ is a
