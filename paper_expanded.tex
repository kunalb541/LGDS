...
Predictive global privilege is equivalent to \emph{$r$-coherence}---all
$M_t$ share a common top-$r$ eigenspace.
Causal global privilege is equivalent to \emph{$r$-coverability}---a
single rank-$r$ subspace nontrivially intersects the leading invariant
subspace of every $K_t$.
...
fork witness exhibiting causal-without-predictive privilege, and a
$3\times 3$ weak-glue counterexample in which the coarse regret bound
overestimates the true regret by a factor of $36$.
\end{abstract}
...
\begin{enumerate}
\item \emph{Predictive global privilege} (Theorem~\ref{thm:pred}):
A common rank-$r$ projection is simultaneously optimal for all
predictive targets $\{M_t\}$ if and only if the family is
\emph{$r$-coherent}, meaning all $M_t$ share a common top-$r$
eigenspace.
This is the structural condition behind the linear Gaussian dynamical
systems (LGDS) iff result; we present it for arbitrary PSD families
and recover the LGDS Bayes-risk instance as a corollary.

\item \emph{Causal global privilege} (Theorem~\ref{thm:caus}):
A common rank-$r$ projection is simultaneously optimal for all causal
targets $\{K_t\}$ under the operator-norm score if and only if the
family is \emph{$r$-coverable}, meaning some rank-$r$ subspace
intersects the leading invariant subspace of every $K_t$
nontrivially.
The proof uses the rank-$1$-driven structure of the
$\lambda_{\max}(P K P)$ maximiser.

\item \emph{The fork} (Theorem~\ref{thm:fork}): For the same PSD family,
$r$-coherence implies $r$-coverability but the converse fails.
A $3\times 3$ witness shows the strict gap: causal global privilege
can hold while predictive global privilege fails.
This dissociation is structural, not a consequence of approximation
error.

\item \emph{Weak-glue regret} (Theorem~\ref{thm:weakglue}): When
coverability fails, the eigenvalue-weighted surrogate
...
and the resulting regret bound is tight in three structural regimes
(rank-$1$ Jacobians, strong glue, common-shape family).
The instance-dependent refinement
$\sum_i w_i \lambda_2(A_i)(1-\|P_F^\star u_i\|^2)$ unifies the
equality conditions and is provably tighter than the coarse
$\sum_i w_i \lambda_2(A_i)$ bound.
\end{enumerate}
...
What is less standard is a structural characterization of when a single
finite-rank projection is exactly optimal across an entire \emph{task family}
under each of the two scoring rules above; the dissociation between
predictive and causal privilege, and the weak-glue surrogate with
instance-dependent regret, appear to be new.

\paragraph{Outline.}
Section~\ref{sec:setup} gives the setup, the two scoring rules, and the
notion of finite global privilege.
...
S^{\mathrm{caus}}_t(P) := \lambda_{\max}(P K_t P).
\end{equation}
The predictive score is the Ky Fan trace; the causal score is the
operator norm of the symmetrically projected operator.
Both are smooth functions of $P\in\Gr(r,d)$.

\paragraph{Finite global privilege.}
\begin{definition}[Finite global privilege]\label{def:fgp}
A rank-$r$ projector $P^\star$ has \emph{finite global privilege} for the
family $\mathcal{T}$ under score $S$ if
$S_t(P^\star) = \max_{\rank P = r} S_t(P)$ for every $t \in \mathcal{T}$.
\end{definition}

The central question of the paper is when finite global privilege exists,
under each scoring rule, and what structure is forced on the family if
so.
...
\begin{theorem}[Predictive global privilege]\label{thm:pred}
The PSD family $\{M_t\}$ admits a rank-$r$ projector with finite global
privilege under $S^{\mathrm{pred}}$ if and only if $\{M_t\}$ is
$r$-coherent.
\end{theorem}
...
\begin{theorem}[Causal global privilege]\label{thm:caus}
The PSD family $\{K_t\}$ admits a rank-$r$ projector with finite global
privilege under $S^{\mathrm{caus}}$ if and only if $\{K_t\}$ is
$r$-coverable.
\end{theorem}

\begin{proof}
By Proposition~\ref{prop:localmax}, $P^\star$ has finite global causal
privilege for $\{K_t\}$ iff $\ran(P^\star)\cap\Emax(K_t)\ne\{0\}$ for
every $t$.
...
\begin{proof}
(i) If $\{T_t\}$ is $r$-coherent with shared rank-$r$ subspace $V^\star$,
then $V^\star$ is contained in a dominant invariant subspace of every
$T_t$.
In particular, $V^\star\cap\Emax(T_t)\ne\{0\}$ for every $t$ since
$\Emax(T_t)$ is contained in the dominant invariant subspace and has
positive dimension.
Hence $V^\star$ is an $r$-cover.
...
intervention response), the two privilege types decouple completely.
Causal global privilege can hold while predictive global privilege fails,
and vice versa: the world seen by a bounded observer depends on the
task functional.
\end{remark}
...
simultaneously.
The relevant question becomes \emph{approximate} causal observer
...
Let $\mu=\sum_{i=1}^N w_i\delta_{x_i}$ with $w_i>0$, $\sum w_i=1$.
At each state $x_i$ let $J_i$ denote a local intervention Jacobian and
$A_i:=J_iJ_i^\top\succeq 0$ the local output Gram with simple top
eigenvalue $\lambda_1(A_i)>\lambda_2(A_i)\ge 0$ and top eigenvector
$u_i$.
Define the residual ratio $\kappa_i:=\lambda_2(A_i)/\lambda_1(A_i)\in[0,1)$.
...
the top-$r$ eigenspace of the rank-$\le N$ matrix $B_\lambda$.
Let $P_F^\star\in\arg\max F$ and $P_B^\star:=$~top-$r$ eigenspace of
$B_\lambda$.
...
\begin{proof}
For each $i$, the Cauchy interlacing/Rayleigh inequality gives
$\lambda_1(A_i)\|Pu_i\|^2\le\lambda_{\max}(PA_iP)\le\lambda_1(A_i)\|Pu_i\|^2
+\lambda_2(A_i)(1-\|Pu_i\|^2)$.
The lower bound proves $\tilde F(P)\le F(P)$:
$\tilde F(P)=\tr(P B_\lambda)=\sum_i w_i\lambda_1(A_i)\|Pu_i\|^2
\le\sum_i w_i\lambda_{\max}(PA_iP)=F(P)$.
The upper bound gives
$F(P)\le\sum_i w_i[\lambda_1(A_i)\|Pu_i\|^2+\lambda_2(A_i)(1-\|Pu_i\|^2)]
\le\tilde F(P)+\sum_i w_i\lambda_2(A_i)$ since $1-\|Pu_i\|^2\le 1$.

For~\eqref{eq:coarse}: $F(P_F^\star)-F(P_B^\star)
=[F(P_F^\star)-\tilde F(P_F^\star)]+[\tilde F(P_F^\star)-\tilde F(P_B^\star)]
+[\tilde F(P_B^\star)-F(P_B^\star)]$.
The middle term is $\le 0$ by surrogate optimality of $P_B^\star$;
the third is $\le 0$ by the lower sandwich; the first is bounded by
$\sum_i w_i\lambda_2(A_i)$ from the upper sandwich.

Pointwise equality $\tilde F=F$ requires
$\lambda_2(A_i)(1-\|Pu_i\|^2)=0$ for all $i$ and all $P$, which forces
$\lambda_2(A_i)=0$ for all $i$, i.e.\ rank-$1$ Jacobians.
...
\begin{proof}
As in the proof of Theorem~\ref{thm:weakglue},
$F(P_F^\star)-F(P_B^\star)\le F(P_F^\star)-\tilde F(P_F^\star)$.
The upper sandwich applied at $P_F^\star$ gives
$\lambda_{\max}(P_F^\star A_i P_F^\star)
\le\lambda_1(A_i)\|P_F^\star u_i\|^2+\lambda_2(A_i)(1-\|P_F^\star u_i\|^2)$,
so
$F(P_F^\star)-\tilde F(P_F^\star)
=\sum_i w_i[\lambda_{\max}(P_F^\star A_i P_F^\star)-\lambda_1(A_i)\|P_F^\star u_i\|^2]
\le\sum_i w_i\lambda_2(A_i)(1-\|P_F^\star u_i\|^2)$.
\end{proof}
...
Optimisation over $\Gr(2,8)$ uses $300$ random initialisations of
projected L-BFGS-B with gradient-norm threshold $10^{-12}$ and
function-value tolerance $10^{-15}$.
...
Causal global privilege exists.
...
$\{M_t\}$ is not $2$-coherent and predictive global privilege fails.
...
\begin{itemize}
\item Under the predictive trace score, finite global privilege
exists iff the family is $r$-coherent (Theorem~\ref{thm:pred}).
\item Under the causal operator-norm score, finite global privilege
exists iff the family is $r$-coverable (Theorem~\ref{thm:caus}).
...
simple-top case.
When neither holds, the regret gap $\Gamma_{\mathrm{pred}}>0$ or the
weak-glue regret $\sum_i w_i\lambda_2(A_i)(1-\|P_F^\star u_i\|^2)$
provides a computable lower bound on unavoidable suboptimality of any
single projection.
$\Gamma=0$ is the certificate of structural alignment between the system
dynamics and the task family; $\Gamma>0$ is the certificate that
dedicated per-target projections are necessary to achieve global
optimum.
