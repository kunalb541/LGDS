...
$r$-coherence is closer to a common-principal-subspace condition
...
In the simple-top case ($\dim\Emax(K_t)=1$ with top eigenvector $u_t$
for every $t$), $V$ must contain each $u_t$, so the problem reduces to
the dimension count $\dim\mathrm{span}\{u_t\}\le r$.
In the general case ($\dim\Emax(K_t)$ possibly $>1$), $r$-coverability
is implied by the simple-top reduction applied to any choice of unit
$u_t\in\Emax(K_t)$ but is in general weaker, since the freedom to choose
$u_t$ within each $\Emax(K_t)$ may bring $\dim\mathrm{span}\{u_t\}$
below $r$.
A constructive test exists when $\dim\sum_t\Emax(K_t)\le r$: any
