...
eigenspace.
This is an exact algebraic condition, not an approximation.
...
optimal observation \sep Grassmann manifold \sep Ky Fan theorem
...
is achieved by the leading eigenspace of $M_{C,\tau}$, recovering PCA
for squared-error prediction \cite{Hotelling1933} as the static ($A=I$)
...
It is the exact condition for a common optimal rank-$r$ observer to exist.
\end{remark}
...
between the system dynamics and the task family.
Outside $r$-coherence, the optimal observer is target-dependent, and the cost
of applying any fixed observer to a non-native target is given exactly by the
Bayes risk formula.
