%% cover_letter.tex
%% Cover letter for submission to Linear Algebra and its Applications
...
coincidence condition on the Grassmannian $\mathrm{Gr}(r,n)$, which is the
literal domain of the optimisation.
The result is not asymptotic and does not rely on approximation.

\paragraph{Fit for the journal.}
The central objects --- symmetric matrices, their eigenspaces, and the
Grassmannian $\mathrm{Gr}(r,n)$ --- place this squarely within the scope of
\textit{Linear Algebra and its Applications}.
The key tool is the Ky Fan variational principle (a classical result in matrix
analysis), and the main theorem is an exact algebraic equivalence.
The paper is self-contained and concise.
MSC 2020 codes: 15A18 (Eigenvalues, singular values, and eigenvectors);
93B07 (Observability); 62H25 (Factor analysis and principal components).
...
Experts in matrix analysis, subspace methods, and the Grassmannian would be
well-placed to evaluate this work.
Specific suggestions:
\begin{itemize}
  \item A specialist in the Ky Fan variational principle and matrix
        inequalities.
  \item A specialist in Grassmannian geometry and orthogonality constraints
        (Edelman, Arias, Smith 1998).
  \item A specialist in linear systems and observability.
\end{itemize}
