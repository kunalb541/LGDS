...
The Ky Fan and Rayleigh--Ritz ingredients are classical
\cite{Fan1949,HornJohnson2013}; the new content of this paper is the
comparative common-optimality framework, the strict
coherence/coverability fork, and the weak-glue value-regret analysis
listed below.

\paragraph{Contribution}
Theorems~\ref{thm:pred} and~\ref{thm:caus} are structural consequences
of classical variational principles, not new variational principles in
their own right.
...
\label{rem:covtest}
$r$-coverability is the question whether there exists an $r$-plane
$V\in\Gr(r,d)$ meeting every $\Emax(K_t)$ nontrivially.
This is a finite-dimensional subspace-transversal / incidence problem
on the Grassmannian: $V$ must satisfy a Schubert-type incidence
condition $V\cap\Emax(K_t)\ne\{0\}$ for every $t$.
We do not pursue a complexity classification; the following practical
guidance suffices for our purposes.
\begin{itemize}
...
\item \emph{Numerical / algebraic test.} Outside these regimes, $r$-
coverability can be tested numerically as feasibility on $\Gr(r,d)$ or
in principle via Schubert incidence calculus; a complete algorithmic
classification is outside scope.
\end{itemize}
...
may bring $\dim\mathrm{span}\{u_t\}$ below $r$ when some
$\Emax(K_t)$ are multidimensional.
\end{remark}
...
We use two informal labels for convenience: \emph{strong glue} for the
simple-top $r$-coverable case ($\dim\mathrm{span}\{u_i\}\le r$), and
\emph{weak glue} for the non-coverable regime in which the surrogate
$B_\lambda$ is used in place of an exact common optimum.
These labels are not additional assumptions; they are shorthand for
regimes already characterised by Definition~\ref{def:cov} and the
