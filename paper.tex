...
\title{Task-Dependent Optimal Linear Projections in
...
projection minimizing squared-error prediction loss for target $Cx_{t+\tau}$
...
rank-$r$ projection.
A common optimal projection exists if and only if the target family is
...
\keywords{linear Gaussian dynamical systems, low-rank linear projections,
optimal prediction, Grassmannian, Ky Fan theorem}
...
a single rank-$r$ linear projection can be simultaneously optimal for predicting
multiple output channels $\{C_k x_{t+\tau_k}\}$, or whether the optimal
projection depends on the specific prediction target and horizon.
For finite-dimensional linear Gaussian dynamical systems (LGDS) at
stationarity, this paper gives an exact answer: the optimal rank-$r$ projection
for predicting $Cx_{t+\tau}$ is the top-$r$ eigenspace of the target
information matrix $M_{C,\tau}$, and a common optimal projection across a family
...
sense \cite{Luenberger1971,OReilly1983,Kailath1980}: the projection is a fixed rank-$r$
...
prediction horizon $\tau$, and projection rank through the information matrix
$M_{C,\tau}$.
...
is one rank-$r$ projection simultaneously optimal for all of them, and when
is target-dependent selection unavoidable?

The Grassmannian $\Gr(r,n)$ is the natural parameter space for rank-$r$
linear projections \cite{Edelman1998}, and projection quality---the trace
$\tr(U M_{C,\tau} U^\top)$---is a smooth function on this manifold.
Optimal projection selection reduces to trace maximisation on $\Gr(r,n)$,
...
question: when does a single rank-$r$ projection remain exactly optimal across
...
shows that any commonly optimal projection must be the unique top-$r$ eigenspace
...
The result is scoped to finite-dimensional LGDS, fixed-rank linear projections,
and squared-error prediction targets.

% -------------------------------------------------------------------------
\section{System and Projection Family}
% -------------------------------------------------------------------------
...
\paragraph{Projection family.}
A rank-$r$ projection is parametrised by a
row-orthonormal matrix $U \in \R^{r \times n}$, $U U^\top = I_r$, via
\begin{equation}
  \phi_U(x) = U \Sigma^{-1/2} x.
\end{equation}
Pre-whitening by $\Sigma^{-1/2}$ places all projections on a common isotropic
footing; in the static case ($A=I$, $\tau=0$) this reduces to principal
component analysis \cite{Hotelling1933}.
Two projections differing only by an orthogonal rotation of their row spaces are
equivalent; the projection space is the Grassmannian
...
(The map $\phi_U$ is also called a \emph{linear observer} in the state-estimation
literature \cite{Luenberger1971}; we use ``projection'' to distinguish it from
dynamic observer synthesis.)
...
eigenvector $e_k$ in the whitened basis: the optimal rank-$1$ projection
...
horizon-dependent optimal projection.
\end{remark}

\subsection{Top-eigenspace optimality}

\begin{proposition}\label{prop:opt}
For fixed rank $r$, the projection minimising $\mathcal{R}(U;\,C,\tau)$ over
...
optimal projections spanning the degenerate eigenspace; in practice, a generic
...
The \emph{task regret} of projection $U$ for target $(C,\tau)$ is the excess
Bayes risk:
...
\subsection{Target-dependent projection selection}

\begin{corollary}\label{cor:noglobal}
If targets $(C_1,\tau_1)$ and $(C_2,\tau_2)$ have information matrices
$M_1$ and $M_2$ with distinct top-$r$ eigenspaces, no single rank-$r$ projection
...
a common top-$r$ eigenspace, no common optimal rank-$r$ projection exists.
...
A common optimal rank-$r$ projection exists if and only if $\mathcal{T}$ is
$r$-coherent.
Under strict $r$-gaps ($\lambda_r(M_k)>\lambda_{r+1}(M_k)$ for all $k$),
$r$-coherence means all $M_k$ share the same top-$r$ eigenspace.
With degeneracies, it means there exists a common $r$-dimensional subspace
within the dominant invariant subspace of each $M_k$.
\end{corollary}

\begin{proof}
(\emph{If})\enspace If $\mathcal{T}$ is $r$-coherent with common subspace
$V^*$, the projection with row space $V^*$ minimises
...
by the Ky Fan maximality condition its row space lies within the dominant
invariant subspace of every $M_k$; hence all $M_k$ share a common
$r$-dimensional subspace and $\mathcal{T}$ is $r$-coherent.
Under strict $r$-gaps the dominant invariant subspace is unique of dimension
$r$, so all $M_k$ share exactly the same top-$r$ eigenspace.
\end{proof}
...
Outside $r$-coherence, the optimal projection is target-dependent.

\begin{remark}
$r$-coherence is an algebraic equality between eigenspaces, not an
approximate or asymptotic condition.
It is the exact condition for a common optimal projection to exist.
\end{remark}
...
\subsection{Minimax shared projection and approximate coherence}
\label{sec:minimax}

For a task family $\mathcal{T}=\{(C_a,\tau_a)\}_{a=1}^m$, define the
\emph{minimax shared projection} and \emph{minimax regret gap}:
...
\end{equation}
$U_{\mathrm{mm}}$ is the best shared rank-$r$ projection in the worst-case sense;
$\Gamma\ge 0$ is the unavoidable worst-case cost of any single projection.
...
\paragraph{Setup.}
State dimension $n=8$, projection rank $r=2$.
...
$\CondAAngleTwo^\circ$, confirming that no single rank-2 projection is
...
The numerically computed minimax projection $U_{\mathrm{joint}}$
...
\begin{ruledtabular}
\begin{tabular}{lcc}
Projection & Regret on $T_1$ & Regret on $T_2$ \\
\hline
...
eigenspace for all $\tau$, so the four optimal projections are identical by the
theorem.
...
The threshold $1.5\varepsilon$ is a heuristic calibration.
In this constructed example, the shared dominant eigenspace remains stable
under the tested perturbations, illustrating that approximate coherence can
be quantified via principal angles (Corollary~\ref{cor:approxcoh}) rather
than requiring exact eigenspace equality.
...
In stationary finite-dimensional LGDS, among rank-$r$ linear projections
$\phi_U(x)=U\Sigma^{-1/2}x$ and squared-error prediction targets
$y=Cx_{t+\tau}$, no common optimal rank-$r$ projection exists unless the
...
between the system dynamics and the task family.
Outside $r$-coherence, no common optimal projection exists, and the cost of
applying any fixed projection to a non-native target is given exactly by the
Bayes risk formula.
In design workflows where a single rank-$r$ map must serve multiple
prediction channels, $\Gamma$ is therefore a computable lower bound on
worst-case suboptimality under any shared projection: if $\Gamma>0$, dedicated
per-target projections are necessary to achieve optimality.
The Grassmannian $\Gr(r,n)$ is the literal domain of this optimisation:
a compact Riemannian manifold on which projection quality is a smooth function,
...
The causal axis---whether the projection optimal for intervention differs from
the projection optimal for prediction---is not addressed here.
...
optimal projection reduces to a single eigendecomposition.
Whether analogous shared-eigenspace conditions characterise common optimal
projections in non-Gaussian or nonlinear systems, and whether intervention
objectives impose additional constraints on projection selection, are open
