...
\begin{enumerate}
\item \emph{Trace-score characterisation} (Theorem~\ref{thm:pred}):
...
\item \emph{Operator-norm characterisation} (Theorem~\ref{thm:caus}):
...
motivation (\S\ref{sec:caus}); ``intervention'' here refers to the
...
We adopt two shorthands: \emph{strong glue} for the simple-top
$r$-coverable case ($\dim\mathrm{span}\{u_i\}\le r$), and \emph{weak
glue} for the non-coverable regime where the surrogate $B_\lambda$
replaces an exact common optimum.
These labels are not additional assumptions; they name regimes already
fixed by Definition~\ref{def:cov} and this section.
...
threshold $10^{-12}$).
All initialisations converged to the same minimax value to numerical
tolerance, with final projected gradient norm below the threshold,
consistent with attainment of the global minimum.
The numerical search is illustrative; the single-target optima are
closed-form, and the minimax value is reported as a numerical
illustration of $\Gamma_{\mathrm{tr}}>0$.
