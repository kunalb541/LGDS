...
The Ky Fan principle and eigenspace optimality are classical.
The contribution here is the exact algebraic condition for when a single
rank-$r$ observer is simultaneously optimal across a task family:
$r$-coherence (Definition~\ref{def:rcoh}), stated as an exact biconditional
in Corollary~\ref{cor:coherence}.
When $r$-coherence fails, target-dependent observer selection is unavoidable,
and the worst-case regret gap $\Gamma>0$ is computable from the closed-form
Bayes risk formula.
